...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    image_files = [f for f in all_files if f.endswith('.bmp')]

    image_files = sorted(image_files)

    # Partition the images into two sets
    images_a = image_files[::2]  
    images_b = image_files[1::2]  

    images_a = [os.path.join(path, img) for img in images_a]
    images_b = [os.path.join(path, img) for img in images_b]
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    return np.array(cutoff_frequencies)
    ### END OF STUDENT CODE ####
    ############################
