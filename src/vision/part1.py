...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    const1 = 1 / (sigma * np.sqrt(2 * np.pi))

    kernel = np.exp( (((x - mean) / sigma)**2) / -2 ) * const1

    kernel = kernel * (1 / np.sum(kernel))
    return kernel.reshape((ksize, 1))
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    return create_Gaussian_kernel_1D(4 * cutoff_frequency + 1, cutoff_frequency) @ create_Gaussian_kernel_1D(4 * cutoff_frequency + 1, cutoff_frequency).T
    ### END OF STUDENT CODE ####
    ############################
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    padWidth = j // 2
    padHeight = k // 2
    padImg = np.pad(image, ((padHeight, padHeight), (padWidth, padWidth), (0, 0)))
    res = np.zeros((mI, nI, cI))
    for depth in range(cI):
      for row in range(padHeight, mI + padHeight):
        for col in range(padWidth, nI + padWidth):
              region = padImg[row - padHeight: row + padHeight + 1, col - padWidth: col + padWidth + 1, depth]
              res[row - padHeight, col - padWidth, depth] = np.sum(region * filter)
    return res
