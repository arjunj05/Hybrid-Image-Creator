...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        kernel = create_Gaussian_kernel_2D(cutoff_frequency=cutoff_frequency)
        numChannels = self.n_channels
        kernel = np.reshape(kernel, shape=(1, cutoff_frequency * 4 + 1, cutoff_frequency * 4 + 1))
        kernel = np.tile(kernel, (numChannels, 1, 1, 1))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        return F.conv2d(x, kernel, padding="same", groups=self.n_channels) 
        ### END OF STUDENT CODE ####
        ############################
    ...
    def forward(
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        kernel = self.get_kernel(cutoff_frequency=int(cutoff_frequency))
        low_frequencies = self.low_pass(image1, kernel)
        high_frequencies = image2 - self.low_pass(image2, kernel)
        #low_frequencies = torch.clamp(low_frequencies,min=0, max=1)
        #high_frequencies = torch.clamp(high_frequencies,min=0, max=1)
